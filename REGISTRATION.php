...
<?php 
if(isset($submit)||isset($name)||isset($user)||isset($email)||isset($pass)||isset($repass))
{
include('conn.php');
...
$query=mysqli_query($conn,"SELECT * FROM accounts WHERE username='$user' or email='$email'")or die("Wrong Query");
if ($submit){
if($user&$pass&$repass){
if(strlen($user)>=8){
if(strlen($user)<=16){
...
if($pass==$repass){
mysqli_query($conn,"INSERT INTO accounts (name,username,email,password) VALUES('$name','$user','$email','$pass')");
